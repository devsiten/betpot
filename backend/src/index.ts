...
export default {
  ...
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext) {
    ...
    try {
      ...
      // 3. AUTO-RESOLVE: Find locked sports events where eventTime has passed (match should be finished)
      // Check for events 3+ hours after eventTime (give time for match to complete + delays)
      const resolveCheckTime = new Date(now.getTime() - 3 * 60 * 60 * 1000); // 3 hours ago

      const eventsToResolve = await db.select()
        .from(events)
        .where(and(
          eq(events.status, 'locked'),
          lte(events.eventTime, resolveCheckTime)
        ));
