...
ticketsRoutes.post('/purchase', zValidator('json', purchaseSchema), async (c) => {
  try {
    ...
    if (data.walletAddress === CONFIG.PLATFORM_WALLET) {
      return c.json({
        success: false,
...
ticketsRoutes.post('/claim-all', zValidator('json', batchClaimSchema), async (c) => {
  console.log('=== CLAIM-ALL ENDPOINT HIT ===');
  ...
  try {
    ...
    if (!user) {
      console.log('CLAIM-ALL: No user in context');
      return c.json({ success: false, error: 'Authentication required' }, 401);
    }

    console.log('CLAIM-ALL: User authenticated:', user.id, user.walletAddress);

    const { walletAddress } = c.req.valid('json');
    console.log('CLAIM-ALL: Target wallet:', walletAddress);
    ...
    // IMPORTANT: Wrap in try-catch so we return success even if DB update fails
    // (money was already sent, so we must tell the user it succeeded)
    ...
    try {
      ...
      console.error('CRITICAL: Payout sent but DB update failed:', dbError);
      dbUpdateSuccess = false;
      // Don't throw - we still need to tell the user the payout succeeded
    }
    ...
    return c.json({
      ...
      data: {
        ...
        walletAddress,
        ticketIds,
        dbUpdateSuccess, // Let frontend know if there was a DB issue
      },
    });
  } catch (error: any) {
