...
import { eq, desc, and, sql, count, sum, or, like } from 'drizzle-orm';
...
ticketsRoutes.post('/purchase', zValidator('json', purchaseSchema), async (c) => {
  try {
    ...
    // Check both exact match and multi-ticket variants (TX-0, TX-1, etc.)
    const existingTicket = await db.query.tickets.findFirst({
      where: or(
        eq(tickets.purchaseTx, data.purchaseTx),
        like(tickets.purchaseTx, `${data.purchaseTx}-%`)
      ),
    });
