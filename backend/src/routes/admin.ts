...
admin.post('/events/from-external', zValidator('json', createFromExternalSchema), async (c) => {
  ...
  try {
    ...
    if (existing) {
      // If the existing event is cancelled, reactivate it instead of delete+recreate
      // This avoids FOREIGN KEY errors from tickets referencing this event
      if (existing.status === 'cancelled') {
        const eventTime = new Date(data.eventTime);
        const now = new Date();
        const lockTime = data.category === 'sports' ? eventTime : new Date(eventTime.getTime() - 24 * 60 * 60 * 1000);
        ...
        await db.update(events)
          .set({
            ...
            category: data.category,
            ticketPrice: data.ticketPrice || parseFloat(c.env.TICKET_PRICE || '10'),
            startTime: now,
            lockTime,
            eventTime,
            status: 'open',
        ...
        await db.delete(eventOptions).where(eq(eventOptions.eventId, existing.id));
        const optionsToInsert = data.options.map((opt, index) => ({
          id: nanoid(),
          eventId: existing.id,
        ...
        const reactivatedEvent = await db.query.events.findFirst({
          where: eq(events.id, existing.id),
          with: { options: true },
        });

        return c.json({ success: true, data: reactivatedEvent, message: 'Event reactivated from cancelled state' }, 201);
      } else {
    ...
    console.log('Inserting event:', { eventId, title: data.title, category: data.category, externalSource: data.externalSource });
