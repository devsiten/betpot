...
admin.post('/events', zValidator('json', createEventSchema), async (c) => {
  ...
  // Check for duplicate/similar events (same title or similar time + category)
  const existingEvent = await db.query.events.findFirst({
    where: or(
      eq(events.title, data.title),
      and(
        eq(events.category, data.category),
        gte(events.eventTime, new Date(eventTime.getTime() - 30 * 60 * 1000)), // within 30 mins
        lte(events.eventTime, new Date(eventTime.getTime() + 30 * 60 * 1000))
      )
    ),
  });
  ...
  if (existingEvent) {
    return c.json({
      success: false,
      error: `Similar event already exists: "${existingEvent.title}"`
    }, 400);
  }
...
admin.post('/events/:id/submit-approval', zValidator('json', approvalSchema), async (c) => {
  try {
    ...
    console.log('Submit approval:', { eventId: id, winningOption, adminId: user.id });
    ...
    console.log('Valid option found:', validOption.optionId);

    // Check if this admin already approved
    let existingApproval = null;
    try {
      existingApproval = await db.query.resolutionApprovals.findFirst({
        where: and(
      ...
      // Table might not exist or query issue - continue without checking
    }
    ...
    // Insert approval
    console.log('Inserting approval...');
    ...
    try {
      allApprovals = await db.query.resolutionApprovals.findMany({
        where: eq(resolutionApprovals.eventId, id),
      });
    } catch (queryError: any) {
      console.error('Error counting approvals:', queryError);
      allApprovals = [];
    }
    ...
    return c.json({
      success: false,
      error: `Failed to submit approval: ${error.message || 'Unknown error'}`,
    }, 500);
  }
});
...
admin.post('/events/:id/resolve', zValidator('json', resolveEventSchema), async (c) => {
  ...
  try {
    // ATOMIC GUARD: Only update if event is still 'locked' (prevents race condition)
    const updateResult = await db.update(events)
      ...
      .where(and(eq(events.id, id), eq(events.status, 'locked')))
      .returning({ id: events.id });

    // If no rows updated, another admin already resolved this event
    if (updateResult.length === 0) {
      return c.json({
        success: false,
        error: 'Event already resolved or status changed. Please refresh and try again.',
      }, 409);
    }
...
admin.post('/failed-transactions/:id/resolve', zValidator('json', z.object({
  ...
  for (let i = 0; i < tx.quantity; i++) {
    ...
    await db.insert(tickets).values({
      ...
      purchaseTx: `RESOLVED-${i}-${tx.transactionSignature}`, // Unique per ticket
      status: 'active',
      createdAt: now,
      updatedAt: now,
    });
...
admin.post('/failed-transactions/:id/refund', zValidator('json', z.object({
  ...
  // Get the actual eventOptions.id - the tx.optionId might be the optionId field (A,B,C) or the id
  let actualOptionId = '';
  let optionLabel = 'Refund';

  // Always look up the option to get the correct primary key ID
  const eventOption = await db.query.eventOptions.findFirst({
    where: tx.optionId
      ? or(eq(eventOptions.id, tx.optionId), eq(eventOptions.optionId, tx.optionId))
      : eq(eventOptions.eventId, tx.eventId),
  });

  if (eventOption) {
    actualOptionId = eventOption.id; // This is the primary key
    optionLabel = `Refund (${eventOption.label})`;
  } else {
    return c.json({ success: false, error: 'No valid option found for this event. Cannot create refund ticket.' }, 400);
  }

  // Calculate SOL amount - use solAmount from tx if available, otherwise from amount
  const solAmount = tx.solAmount || tx.amount;
  ...
  await db.insert(tickets).values({
    ...
    solAmount: solAmount,
    payoutSolAmount: solAmount, // Refund same SOL amount
    purchaseTx: `REFUND-${tx.transactionSignature}`, // Unique refund tx ID
...
admin.get('/users/lookup', async (c) => {
  ...
  try {
    ...
    // Calculate stats - use solAmount for actual SOL values
    ...
    // Use solAmount if available, otherwise use purchasePrice (legacy)
    const totalSpent = userTickets.reduce((sum, t) => sum + (t.solAmount || t.purchasePrice || 0), 0);
    const totalWinnings = userTickets
      .filter(t => t.status === 'claimed')
      .reduce((sum, t) => sum + (t.payoutSolAmount || t.payoutAmount || 0), 0);
    const unclaimedWinnings = userTickets
      .filter(t => t.status === 'won' || t.status === 'refunded')
      .reduce((sum, t) => sum + (t.payoutSolAmount || t.payoutAmount || 0), 0);
    ...
    return c.json({
      ...
      data: {
        ...
        tickets: userTickets.map(t => ({
          ...
          solAmount: t.solAmount || t.purchasePrice, // Include SOL amount
          purchaseTx: t.purchaseTx,
          status: t.status,
          payoutAmount: t.payoutAmount,
          payoutSolAmount: t.payoutSolAmount || t.payoutAmount, // Include SOL payout
...
// ============================================================================
// TWITTER VERIFICATIONS (Admin panel for manual Twitter verification)
// ============================================================================

// Get all Twitter verifications pending review
admin.get('/twitter-verifications', async (c) => {
  const db = c.get('db');
  const { status = 'pending' } = c.req.query();

  try {
    // Import twitterVerifications table
    const { twitterVerifications } = await import('../db/schema');

    const verifications = await db.select({
      id: twitterVerifications.id,
      userId: twitterVerifications.userId,
      twitterHandle: twitterVerifications.twitterHandle,
      status: twitterVerifications.status,
      createdAt: twitterVerifications.createdAt,
      reviewedAt: twitterVerifications.reviewedAt,
      adminNote: twitterVerifications.adminNote,
    })
      .from(twitterVerifications)
      .where(eq(twitterVerifications.status, status))
      .orderBy(desc(twitterVerifications.createdAt));
    ...
    const enrichedVerifications = await Promise.all(verifications.map(async (v) => {
      const user = await db.query.users.findFirst({
        ...
        columns: { walletAddress: true, email: true, displayName: true },
      });
      return { ...v, user };
    }));
    ...
    console.error('Error fetching Twitter verifications:', error);
    return c.json({ success: false, error: 'Failed to fetch verifications' }, 500);
  }
});

// Approve Twitter verification
admin.post('/twitter-verifications/:id/approve', zValidator('json', z.object({
  note: z.string().optional(),
})), async (c) => {
  const db = c.get('db');
  const user = c.get('user')!;
  ...
  try {
    const { twitterVerifications, referrals } = await import('../db/schema');
    const now = new Date();

    // Get the verification
    const verification = await db.select().from(twitterVerifications).where(eq(twitterVerifications.id, id)).limit(1);
    ...
    if (verification[0].status !== 'pending') {
      return c.json({ success: false, error: 'Verification already processed' }, 400);
    }

    // Update verification status
    await db.update(twitterVerifications).set({
      status: 'approved',
      reviewedBy: user.id,
      reviewedAt: now,
      adminNote: note,
    }).where(eq(twitterVerifications.id, id));

    // Update user's Twitter verified status
    await db.update(users).set({
      twitterVerified: true,
      updatedAt: now,
    }).where(eq(users.id, verification[0].userId));

    // Update any referral records for this user
    await db.update(referrals).set({
      twitterVerified: true,
    }).where(eq(referrals.referredUserId, verification[0].userId));

    // Check if both Discord and Twitter are verified for awarding points
    const referral = await db.query.referrals.findFirst({
      where: eq(referrals.referredUserId, verification[0].userId),
    });

    if (referral && referral.discordVerified && !referral.pointsAwarded) {
      // Award 100 points to the referrer
      await db.update(users).set({
        referralPoints: sql`${users.referralPoints} + 100`,
        updatedAt: now,
    ...
    return c.json({
      success: true,
      data: { message: 'Twitter verification approved' },
    });
  } catch (error: any) {
    console.error('Error approving Twitter verification:', error);
    return c.json({ success: false, error: 'Failed to approve verification' }, 500);
  }
});

// Reject Twitter verification
admin.post('/twitter-verifications/:id/reject', zValidator('json', z.object({
  reason: z.string(),
})), async (c) => {
  const db = c.get('db');
  const user = c.get('user')!;
  ...
  try {
    ...
    return c.json({
      success: true,
      data: { message: 'Twitter verification rejected' },
    });
  } catch (error: any) {
    console.error('Error rejecting Twitter verification:', error);
