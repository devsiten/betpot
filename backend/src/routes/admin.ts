...
admin.post('/failed-transactions/:id/refund', zValidator('json', z.object({
  ...
  // Get the actual optionId - if not on failed tx, get first option from event
  let optionId = tx.optionId;
  let optionLabel = 'Refund';

  if (!optionId) {
    // Get the first option from the event as fallback
    const eventOption = await db.query.eventOptions.findFirst({
      where: eq(eventOptions.eventId, tx.eventId),
    });
    if (eventOption) {
