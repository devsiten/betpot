...
// ============================================================================
// GET MATCH RESULT - Search for completed/live match by team names
// ============================================================================

sportsApi.get('/match-result', async (c) => {
    const apiKey = c.env.API_SPORTS_KEY;
    const title = c.req.query('title') || '';

    if (!title) {
        return c.json({ success: false, error: 'Title parameter required' }, 400);
    }

    // Parse team names from title (e.g., "Arsenal vs Chelsea" -> ["Arsenal", "Chelsea"])
    const teamMatch = title.match(/(.+?)\s+vs\.?\s+(.+)/i);
    if (!teamMatch) {
        return c.json({ success: false, error: 'Could not parse team names from title' }, 400);
    }
    ...
    try {
        ...
        if (liveResponse.ok) {
            ...
            const liveMatch = liveFixtures.find((f: any) => {
            ...
            if (liveMatch) {
                ...
                let winner = 'Draw';
                if (homeScore > awayScore) winner = 'Home Win';
                else if (awayScore > homeScore) winner = 'Away Win';
                ...
                return c.json({
                    ...
                    data: {
                        ...
                        homeScore,
                        awayScore,
                        score: `${liveMatch.teams.home.name} ${homeScore} - ${awayScore} ${liveMatch.teams.away.name}`,
                        winner,
                    }
                });
            }
        ...
        // Try finished matches from today/yesterday
        const today = new Date().toISOString().split('T')[0];
        ...
        if (finishedResponse.ok) {
            const finishedData: any = await finishedResponse.json();
            const finishedFixtures = finishedData.response || [];

            const finishedMatch = finishedFixtures.find((f: any) => {
                const home = f.teams.home.name.toLowerCase();
                const away = f.teams.away.name.toLowerCase();
                return (home.includes(searchHome) || searchHome.includes(home.split(' ')[0])) &&
                    (away.includes(searchAway) || searchAway.includes(away.split(' ')[0]));
            ...
            if (finishedMatch) {
                const homeScore = finishedMatch.goals.home ?? 0;
                const awayScore = finishedMatch.goals.away ?? 0;
                let winner = 'Draw';
                if (homeScore > awayScore) winner = 'Home Win';
                else if (awayScore > homeScore) winner = 'Away Win';
                ...
                return c.json({
                    success: true,
                    data: {
                        status: finishedMatch.fixture.status.long,
                        isLive: false,
                        elapsed: null,
                        ...
                        homeScore,
                        awayScore,
                        score: `${finishedMatch.teams.home.name} ${homeScore} - ${awayScore} ${finishedMatch.teams.away.name}`,
                        winner,
                    }
                });
            }
        ...
        // Try yesterday if not found today
        ...
        if (yesterdayResponse.ok) {
            ...
            const yesterdayFixtures = yesterdayData.response || [];

            const match = yesterdayFixtures.find((f: any) => {
                const home = f.teams.home.name.toLowerCase();
                const away = f.teams.away.name.toLowerCase();
                return (home.includes(searchHome) || searchHome.includes(home.split(' ')[0])) &&
                    (away.includes(searchAway) || searchAway.includes(away.split(' ')[0]));
            ...
            if (match) {
                ...
                return c.json({
                    ...
                    data: {
                        ...
                        homeScore,
                        awayScore,
                        score: `${match.teams.home.name} ${homeScore} - ${awayScore} ${match.teams.away.name}`,
                        winner,
                    }
                });
            }
        ...
        return c.json({
            ...
            searched: { home: searchHome, away: searchAway }
        }, 404);

    } catch (error: any) {
        console.error('Match result search error:', error);
        return c.json({ success: false, error: error.message || 'Failed to fetch match result' }, 500);
    }
});
